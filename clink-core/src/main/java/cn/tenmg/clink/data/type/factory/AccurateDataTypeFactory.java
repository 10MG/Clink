...
public abstract class AccurateDataTypeFactory extends AbstractDataTypeFactory {
	 ...
	 */
	abstract DataType create(int scale, int precision);
	...
	public DataType create(String desc) {
		int scaleStart = desc.indexOf(SQLUtils.LEFT_BRACKET) + 1;
		if (scaleStart > 0) {
			int splitIndex = desc.indexOf(",");
			if (splitIndex > 0) {
				int precisionEnd = desc.indexOf(SQLUtils.RIGTH_BRACKET), next = precisionEnd + 1;
				DataType dataType = create(Integer.parseInt(desc.substring(scaleStart, splitIndex).trim()),
						Integer.parseInt(desc.substring(splitIndex + 1, precisionEnd)));
				...
				DataType dataType = create(Integer.parseInt(desc.substring(scaleStart, scaleEnd).trim()));
				if (next < desc.length() && notNull(desc.substring(next))) {
					return dataType.notNull();
				}
				return dataType;
